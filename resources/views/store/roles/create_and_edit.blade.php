    ...
    <div class="row bg-title">
        ...
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
            <ol class="breadcrumb">
                ...
                <li><a href="{{ route('store.roles.index') }}">角色列表</a></li>
        ...
        <div class="white-box">
            <div class="row">
                <div class="col-sm-12 col-xs-12">
                    @if ($role->id)
                        <form method="POST"
                              action="{{ route('store.roles.update', $role->id) }}">
                            {{ method_field('PUT') }}
                            @else
                                <form method="POST"
                                      action="{{ route('store.roles.store') }}">
                                    @endif
                                    {{ csrf_field() }}
                                    <div class="form-group">
                                        <label for="exampleInputEmail1">角色名称</label>
                                        <input type="text" class="form-control" name="name"
                                               value="{{ old('name', $role->name) }}" placeholder="请输入角色名称">
                                    </div>
                                    <div class="form-group">
                                                        ...
                                                        <td>
                                                            @foreach ($node['actions'] as $key=>$action)
                                                                <div class="checkbox checkbox-primary ">
                                                                    ...
                                                                    @if ($role->id)
                                                                        <input id="checkbox{{ $node['name'] . $key }}"
                                                                               name="node_ids[]"
                                                                               value="{{ $action['id'] }}"
                                                                               type="checkbox"
                                                                               @if(in_array($action['id'], $role['node_ids'])) checked @endif>
                                                                    @else
                                                                        <input id="checkbox{{ $node['name'] . $key }}"
                                                                               name="node_ids[]"
                                                                               value="{{ $action['id'] }}"
