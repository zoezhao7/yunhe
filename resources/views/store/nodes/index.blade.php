...
@section('content')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="white-box">
                        <div class="card-body">
                            <!-- <a class="btn btn-info btn-sm float-right" href="{{ route('store.nodes.create') }}">创建节点</a> -->
                                ...
                                <table id="demo-foo-addrow" class="table table-bordered m-t-10 table-hover contact-list" data-paging="true" data-paging-size="7">
                                    ...
                                    @foreach($nodes as $key => $node)
                                        <tr>
                                            ...
                                            <td>{{ $node->action }}</td>
                                            <td>
<!--                                                 <a href="{{ route('store.nodes.edit', $node->id) }}" class="btn btn-sm btn-outline btn-info m-r-10">编辑</a>

<form  method="post" action="{{ route('store.nodes.destroy', $node->id) }}" style="display: inline">
    {{ csrf_field() }}
    {{ method_field('DELETE') }}
