...
<div class="pageIndex">
    <div class="">
        ...
        <div class="car_swiper_container">
            <div class="swiper-wrapper">
...
<script>
    ...
    $.get('{{ route('tools.car_demos.cars') }}', function (res) {
        ...
        $.get('{{ route('tools.car_demos.hubs') }}', function (res) {
            ...
            $.each(hubs, function (i, c) {
                var html = '<div class="swiper-slide swiper_box" onclick="selectWheel(' + c + ',' + i + ')">\
    ...
    function selectWheel(c, i) {
        $('.wheel_swiper_container .swiper_box').eq(i).addClass('on').siblings().removeClass('on');
        $('.wheel_box1 img').attr('src', c.image);
        $('.wheel_box2 img').attr('src', c.image);
    }
</script>
</body>
</html>