                ...
                <table class="table store-overview" id="myTable">
                    ...
                    @foreach ($stores as $key => $store)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('admin.stores.edit', $store->id)  }}" class="text-inverse p-r-10" data-toggle="tooltip" title="Edit"><i class="ti-marker-alt"></i></a>
                                <form onsubmit="return confirm('确认删除吗？');" id="delete_form" method="post" action="{{ route('admin.stores.destroy', $store->id) }}" style="display: inline">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
